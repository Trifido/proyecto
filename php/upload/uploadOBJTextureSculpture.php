...
<?php
	...
	$query = 'SELECT*FROM TexturasEsculturas WHERE NombreEscultura=\'' . $nombreEscultura . '\' AND pathTextura=\'' . $path_file . '\' AND TipoTextura=\'OBJ\'';
	$result = sqlite_query($db, $query);
	$array = $result->fetchArray();

	if($nombreEscultura != $array["NombreEscultura"]){
	    $query = 'INSERT INTO TexturasEsculturas (NombreEscultura, pathTextura, TipoTextura) 
				  VALUES (\'' . $nombreEscultura,  . '\', \''.$path_file.'\', \'OBJ\')';
	    $result = sqlite_query($db, $query);
